...
# 2. DB (사용) 활성화, 여러개 DB가 존재하므로 사용 DB 선택
use mydb1126;
# 3. 테이블 생성, create table 테이블명(필드명 타입 제약조건, 필드명2 타입 제약조건, 필드명3 제약조건);
create table board(
	num int unsigned auto_increment , 
    # 게시물 번호, num 필드명으로 int 타입 선정, unsigned:부호 없음, auto_increment:레코드 삽입시 자동번호 할당
    content longtext not null , 
    # 게시물 내용, content 필드명으로 longtext 타입 선정, not null:공백 없음
    writer varchar(20) not null , 
    # 게시물 작성자, writer 필드명으로 varchar(20) 가변길이 타입 선정, 공백 없음
    pwd int not null , 
    # 게시물 비밀번호, pwd 필드명으로 int 타입 선정, 공백 없음
    primary key(num)
    # 식별(pk)키는 중복값이 없고 고유한 값의 특성, 예] 주민등록번호, 사번, 학번, ISBN(도서번호), 제품번호
    # 어떠한 형태물(객체) 식별 할 때 절대 중목이 없는 고유한 성질
    # PK(식별키) <----> FK(참조키/외래키:pk필드를 참조하는 필드)
);
